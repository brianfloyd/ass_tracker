...
$('body').on("click","button",function(){
  ...
  $('body').on("click",".card",function(){
    ...
    if($('.add_ass').length>0){
        $('.card img').remove('.add_ass').remove('.minus_ass');
    }


var cardPicked = $("."+($(this)[0].classList[1]));
var check = ($(this)[0].classList[1]);

if (check !="new_store"){
$(iconLib.add).appendTo(cardPicked);
$(iconLib.minus).appendTo(cardPicked);
$('.add_ass , .minus_ass'). on ('click', function(){
    var selector = ($(this)[0].classList[0]);
